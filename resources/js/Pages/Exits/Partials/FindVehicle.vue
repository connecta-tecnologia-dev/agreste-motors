...
const selectVehicle = (h) => {
    if(form.vehicle != h.entry.vehicle.id){
        form.vehicle = h.entry.vehicle.id;
    }else{
        form.vehicle = '';
    }
};
...
const loadVehicles = async (queryParams = {}) => {
    try {
        ...
        const response = await axios.get(route('showStock'), queryParams);
                    ...
                    <table v-if="data.vehicles.length > 0"
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="dataVehicle in data.vehicles" v-on:exit-has-updated="loadExits"
                                ...
                                <td :class="{ 'bg-green-400': form.vehicle == dataVehicle.entry.vehicle.id }"
                                    class="px-6 py-4 cursor-pointer border border-black">{{ dataVehicle.entry.id }}</td>
                                <td :class="{ 'bg-green-400': form.vehicle == dataVehicle.entry.vehicle.id }"
                                    class="px-6 py-4 cursor-pointer border border-black">{{
                                        dataVehicle.entry.vehicle.brand.name }}</td>
                                <td :class="{ 'bg-green-400': form.vehicle == dataVehicle.entry.vehicle.id }"
                                    class="px-6 py-4 cursor-pointer border border-black">{{
                                        dataVehicle.entry.vehicle.model.name }}</td>
                                <td :class="{ 'bg-green-400': form.vehicle == dataVehicle.entry.vehicle.id }"
                                    class="px-6 py-4 cursor-pointer border border-black">{{
                                        dataVehicle.entry.vehicle.version }}</td>
                                <td :class="{ 'bg-green-400': form.vehicle == dataVehicle.entry.vehicle.id }"
                                    class="px-6 py-4 cursor-pointer border border-black">{{ dataVehicle.entry.vehicle.plate
                                    }}</td>
                                <td :class="{ 'bg-green-400': form.vehicle == dataVehicle.entry.vehicle.id }"
                                    class="px-6 py-4 cursor-pointer border border-black">{{
                                        dataVehicle.entry.vehicle.color.name }}</td>
                            </tr>
                        </tbody>
                    </table>
