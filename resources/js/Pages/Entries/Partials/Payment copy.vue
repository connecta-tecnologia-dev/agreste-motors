...
const returnData = async () => {
    const payment = { formPayment: form.payment, value: form.value };
                ...
                <div class="w-60 flex flex-col gap-1">
                    ...
                    <div class="flex w-full gap-3 items-center">
                        <Multiselect class="w-full" placeholder="Forma de Pagamento" v-model="form.payment"
                            @select="selectPayment" :searchable="true" :canClear="false" :options="async (query) => {
                                return await loadPayments(query)
                            }" />
                    </div>
                </div>
                ...
                <template v-if="payments.selected && payments.selected.requirements.length > 0">
                    <template v-for="requirement in payments.selected.requirements">
                        <div class="w-48 flex flex-col gap-1" v-if="requirement.options.type == 'bank'">
                            ...
                            <Multiselect class="w-full" placeholder="Banco" v-model="form.requirements[requirement.options.slug]"
